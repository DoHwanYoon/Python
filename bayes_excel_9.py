...
for i in range(2, data_cols):
    test_d = test_data_ws.cell(2, i).value in bayes_value_dic_0
    if test_d == True:
        #print(test_data_ws.cell(2, i).value)
        result_0 *= bayes_value_dic_0[test_data_ws.cell(2, i).value]
        
for i in range(2, data_cols):
    test_d = test_data_ws.cell(2, i).value in bayes_value_dic_1
    if test_d == True:
        #print(test_data_ws.cell(2, i).value)
        result_1 *= bayes_value_dic_1[test_data_ws.cell(2, i).value]

'''
print()
print("결과")   
print(test_data_ws.cell(2, 1).value ,"|", result_list[0],"=",result_0*result_count_dic[result_list[0]])
print(test_data_ws.cell(2, 1).value ,"|", result_list[1],"=",result_1*result_count_dic[result_list[1]])
'''
lb1 = Label(root, text = "결과 확률 : "+ str(bayes_result_dic))
lb1.pack()

lb2 = Label(root, text = result_list[0]+"의 확률 = "+ str(bayes_value_dic_0))
lb2.pack()

lb3 = Label(root, text = result_list[1]+"의 확률 = "+ str(bayes_value_dic_1))
lb3.pack()

lb4 = Label(root, text = "결과")
lb4.pack()

lb5 = Label(root, text = test_data_ws.cell(2, 1).value +"|"+ result_list[0]+"="+str(result_0*result_count_dic[result_list[0]]))
lb5.pack()

lb6 = Label(root, text = test_data_ws.cell(2, 1).value +"|"+ result_list[1]+"="+str(result_1*result_count_dic[result_list[1]]))
lb6.pack()
